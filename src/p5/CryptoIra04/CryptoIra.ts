...
export class CryptoIra {
    ...
    constructor(p: p5, freq: number, boundary: p5.Vector) {
        ...
        for (let i = 0; i < this.data.length; i += 3) {
            const radScaler = this.p.random(-.001, .001);
            this.vBlocks.push(
                new VerletNode(
                    p,
                    ...
                    p.color(this.cols[i] + this.p.int(this.p.random(-15, 15)), this.cols[i + 1] + this.p.int(this.p.random(-15, 15)), this.cols[i + 2] + this.p.int(this.p.random(-15, 15))))); // color
        }

        for (let i = 0, k = 0; i < this.vBlocks.length; i++) {
            for (let j = i; j < this.vBlocks.length; j++) {
                if (k++ % this.p.int(this.p.random(1, 4)) == 0 && i !== j) {
                    this.vSticks.push(new VerletStick(this.p, this.vBlocks[i], this.vBlocks[j], this.p.random(.0006, .001)));
                }
            }
        }
    ...
    draw() {
        ...
        if (this.p.frameCount % 1 === 0) {
            this.vBlocks[this.p.int(this.p.random(this.vBlocks.length))].nudge(new p5.Vector(this.p.random(-.01, .01), this.p.random(-.01, .01), this.p.random(-.01, .01)));
        }
    }

    nudge(): void {
        for (let i = 0; i < this.vBlocks.length; i++) {
            this.vBlocks[i].nudge(new p5.Vector(this.p.random(-.02, .02), this.p.random(-.02, .02), this.p.random(-.02, .02)));
        }
    }

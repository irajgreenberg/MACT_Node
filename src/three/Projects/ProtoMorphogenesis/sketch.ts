...
for (let i = 0; i < imageMaps.length; i++) {
    ...
    if (imageMaps[i].hasTendrils) {
        protoOrgs[i] = new ProtoOrganism(new Vector3(randFloat(-posX, posX), randFloat(-posY, posY), randFloat(-posZ, posZ)), vSurf, new Color(randFloat(.3, .7), randFloat(.3, .7), randFloat(.3, .7)), pPhys, tdm);
    } else {

...
import { AmbientLight, Color, DirectionalLight, DoubleSide, FogExp2, HemisphereLight, MeshBasicMaterial, MeshLambertMaterial, MeshPhongMaterial, PCFSoftShadowMap, PerspectiveCamera, PointLight, Raycaster, Scene, SpotLight, Texture, TextureLoader, Vector2, Vector3, WebGLRenderer } from 'three'
...
import { NodeSelector, ProtoOrganism_Structure } from './ProtoMorphogenesis_Structure';
...
document.title = "[Proto]morphogenesis_Structure] | Ira Greenberg.2024"
...
let protoOrg: ProtoOrganism_Structure;
...
const matMid = new MeshPhongMaterial({ color: 0xffffff, transparent: true, wireframe: true, flatShading: false, specular: 0x334433, shininess: 250, opacity: randFloat(1, 1), side: DoubleSide, map: textureMid, bumpScale: 5, bumpMap: textureMid });

const matTop = new MeshPhongMaterial({ color: 0xffffff, transparent: true, wireframe: true, flatShading: false, specular: 0x334433, shininess: 250, opacity: randFloat(1, 1), side: DoubleSide, map: textureMid, bumpScale: 5, bumpMap: textureMid });

const sz = randFloat(700, 700);
const detail01 = randInt(12, 18);
const detail02 = randInt(6, 6);
const vSurfMid = new VerletSurface(new Vector2(sz, sz), new Vector2(detail01, detail02), matMid, randFloat(.01, .08), true);

// console.log((sz / 2 - sz / 2 / detail01 * 2);
...
const vSurfTop = new VerletSurface(new Vector2(sz * .5, sz * .5), new Vector2(detail01, detail02), matTop, randFloat(.1, .8), true);
...
async function processImageData(url: string): Promise<void> {
    try {
        ...
        protoOrg = new ProtoOrganism_Structure(new Vector3(randFloat(-posX, posX), randFloat(-posY, posY), randFloat(-posZ, posZ)), vSurfMid, new Color(1, .6, .6), pPhys);
        ...
        pp.start();
        scene.add(pp);
        protoOrg.setSurfaceDrawable(true, true);
...
// Call the function
//processImageData("data/ProtoMorph/imgMap_10x10_all_0_alpha.png");
processImageData("data/ProtoMorph/imgMap_10x10.png");

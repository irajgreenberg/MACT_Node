...
import { NodeSelector, ProtoOrganism_Test } from './ProtoOrganism_Test';
...
document.title = "[Proto]morphogenesis_Test] | Ira Greenberg.2024"
...
const textureMid = new TextureLoader().load('data/ProtoMorph/Proto_Org_001.png');
...
async function processImageData(url: string): Promise<void> {
    try {
        ...
        protoOrg = new ProtoOrganism_Test(new Vector3(randFloat(-posX, posX), randFloat(-posY, posY), randFloat(-posZ, posZ)), vSurfMid, new Color(1, .6, .6), pPhys, tdm, data);
